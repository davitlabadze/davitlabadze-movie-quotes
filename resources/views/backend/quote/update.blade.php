...
<form action="{{ route('quotes.update',['quote' => $quote->id])}}" method="POST" class="mt-10" enctype="multipart/form-data">
    @csrf
    @method('PATCH')
    ...
    <div class="mb-6">
        ...
        <input class="w-full p-2 border border-gray-400" type="text" name="quote[{{ $locale }}]"  id="quote_{{ $locale }}" value="{{ $quote->getTranslation('quote',$locale) }}" />
        @error('quote.' . $locale)
        ...
        <select name="movie_id" id="movie_id">
            @foreach ($movie as $movie)
                <option value="{{ $movie->id }}"
                    ...
                    > {{ $movie->getTranslation('movie','en') }}</option>
            @endforeach
            </div>
        </select>
    </div>
    <div class="flex mb-6" >
       <div class="p-2 border border-gray-500 rounded ">
            ...
            <input type="file" name="thumbnail" value="{{ $quote->thumbnail }}" id="thumbnail">
            @error('thumbnail')
                <p class="mt-2 text-xs text-red-500">{{ $message }}</p>
            @enderror
       </div>
        <img src="{{ asset('storage/'. $quote->thumbnail) }}"  width="64" height="64" class="ml-2" alt="image">
    </div>
